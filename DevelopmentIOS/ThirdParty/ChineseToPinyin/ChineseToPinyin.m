...
{
    NSArray *letters = [ChineseToPinyin stringSplit:string];
    NSString *sp_name = @"";
    for (NSString *letter in letters) {
        NSString *pinyin = [ChineseToPinyin pinyinFromChineseString:letter];
        if (pinyin.length) {
            sp_name = [sp_name stringByAppendingString:[pinyin substringToIndex:1]];
        }
    }
    return sp_name;
}
...
{
    NSMutableArray *stringSplit = [[NSMutableArray alloc] init];
    for (int i = 0; i < string.length; i ++) {
        [stringSplit addObject:[string substringWithRange:NSMakeRange(i, 1)]];
    }
    return stringSplit;
